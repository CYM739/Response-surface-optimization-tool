...
def _format_fixed_vars(fixed_vars_dict, variable_descriptions):
    return ", ".join([f"{variable_descriptions.get(k, k)}={v}" for k, v in fixed_vars_dict.items()])

_DEFAULT_SYNERGY_COLORSCALE = [
    [0.0,  "rgb(200,  50,  50)"],
    [0.45, "rgb(240, 180, 180)"],
    [0.45, "rgb(220, 220, 220)"],
    [0.55, "rgb(220, 220, 220)"],
    [0.55, "rgb(180, 240, 180)"],
    [1.0,  "rgb( 50, 200,  50)"],
]
...
def plot_response_surface(dataframe, OLS_model_1, all_alphabet_vars, x_var, y_var,
                          ...
                          point_symbol_2='cross', point_color_2='#00FF00', z_range=None,
                          # Pre-computed grid override (skips internal predict_surface call)
                          x_grid_override=None, y_grid_override=None, z_grid_override=None,
                          # Separate surface colour axis (e.g. synergy scores)
                          surfacecolor=None, surfacecolor_colorscale=None,
                          surfacecolor_cmin=None, surfacecolor_cmax=None,
                          surfacecolor_label=None,
                          **kwargs):

    if x_grid_override is not None:
        x_grid_1, y_grid_1, z_grid_1 = x_grid_override, y_grid_override, z_grid_override
        x_min, x_max = float(x_grid_1.min()), float(x_grid_1.max())
        y_min, y_max = float(y_grid_1.min()), float(y_grid_1.max())
    else:
        x_min, x_max = dataframe[x_var].min(), dataframe[x_var].max()
        y_min, y_max = dataframe[y_var].min(), dataframe[y_var].max()
        x_grid_1, y_grid_1, z_grid_1 = predict_surface(
            OLS_model_1, all_alphabet_vars, x_var, y_var,
            fixed_vars_dict_1, (x_min, x_max), (y_min, y_max)
        )
    ...
    if show_surface_grid:
        ...
        contours_config = {
            "x": {"show": True, "start": x_min, "end": x_max, "size": x_step,
                  "color": "black", "width": 1, "highlight": False},
            "y": {"show": True, "start": y_min, "end": y_max, "size": y_step,
                  "color": "black", "width": 1, "highlight": False}
        }
    ...
    if surfacecolor is not None:
        trace1 = go.Surface(
            ...
            colorscale=surfacecolor_colorscale or _DEFAULT_SYNERGY_COLORSCALE,
            cmin=surfacecolor_cmin if surfacecolor_cmin is not None else -0.5,
            cmax=surfacecolor_cmax if surfacecolor_cmax is not None else 0.5,
            colorbar=dict(title=surfacecolor_label or 'Synergy'),
            opacity=0.9,
    ...
    if OLS_model_2 and z_var_2 and fixed_vars_dict_2 is not None:
        x_grid_2, y_grid_2, z_grid_2 = predict_surface(OLS_model_2, all_alphabet_vars, x_var, y_var, fixed_vars_dict_2, (x_min, x_max), (y_min, y_max))
        trace2 = go.Surface(
    ...
    title_fixed_vars = _format_fixed_vars(fixed_vars_dict_1, variable_descriptions)
...
def plot_tradeoff_analysis(model_1, model_2, model_name_1, model_name_2, all_alphabet_vars,
    ...
    title_fixed_vars = _format_fixed_vars(fixed_vars_dict, variable_descriptions)
...
def calculate_synergy_surface_grid(model, drug1, drug2, fixed_vars, range1, range2, method='hsa'):
    ...
    """
    # 1. Create Grid using shared utility
    x_grid, y_grid, predict_df = generate_prediction_grid(model, drug1, drug2, fixed_vars, range1, range2, n=50)
